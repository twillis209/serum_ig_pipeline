...
rule subset_ig_pair_for_coloc_pair:
    input:
    ...
    output:
        temp("results/coloc/{first_isotype}_and_{second_isotype}/{first_rsid}_and_{second_rsid}/sumstats.tsv")
    params:
        flank = 250000
    threads: 8
    conda: env_path("global.yaml")
...
rule run_coloc_for_ig_pair:
    ...
    output:
        "results/coloc/{first_isotype}_and_{second_isotype}/{first_rsid}_and_{second_rsid}/coloc.rds"
    params:
...
rule run_coloc_for_all_ig_pairs:
    input:
        iga_igg = lambda w: [f"results/coloc/iga_and_igg/{first_rsid}_and_{second_rsid}/coloc.rds" for first_rsid, second_rsid in get_rsids_from_merged_lead_snps(w, isotype_a = 'iga', isotype_b = 'igg')]
    output:
        "results/coloc/all_ig_pairs.tsv"
    shell: "touch {output}"
#        iga_igm = get_rsids_from_merged_lead_snps(isotype_a = 'iga', isotype_b = 'igm'),
#        igg_igm = get_rsids_from_merged_lead_snps(isotype_a = 'igg', isotype_b = 'igm'),
