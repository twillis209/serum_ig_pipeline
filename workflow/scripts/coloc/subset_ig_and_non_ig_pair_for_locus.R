...
leads <- leads[rsid_a == snakemake@wildcards$isotype_rsid & rsid_b == snakemake@wildcards$non_ig_rsid, env = list(rsid_a = paste('rsid', iso, sep = '.'), rsid_b = paste('rsid', non_ig, sep = '.'))]
if(leads[, .N] == 0) {
