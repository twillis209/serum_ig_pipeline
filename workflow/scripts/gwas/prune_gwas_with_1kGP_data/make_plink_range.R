...
gwas_dat <- fread(gwas_file, sep = '\t', header = T, select = c(chr_col, bp_col, ref_col, alt_col))
...
bim_join <- merge(bim_dat, gwas_dat, by.x = c('CHR38', 'BP38'), by.y = c(chr_col, bp_col), sort = F)
