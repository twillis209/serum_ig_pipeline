...
chrom <- as.character(snakemake@config$loci$igk$chrom)
locus_range <- c(snakemake@config$loci$igk$start, snakemake@config$loci$igk$stop)
...
pls <- lapply(c("eldjarn"), draw_ig_locus_scatter_plot, isotype = "igg")

study <- "eldjarn"
p_value_label <- sprintf("p_value.%s.igg", study)
